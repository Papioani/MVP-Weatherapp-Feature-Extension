...
function Login() {
  ...
  const login = async () => {
    try {
      const { data } = await axios("/api/auth/login", {    /* this gets us to ...auth.js line 32 > router.post("/login", async (req, res) => { etc... */
        method: "POST",
  ...
  const requestData = async () => {
    try {
      const { data } = await axios("/api/auth/profile", {
        headers: {
          authorization: "Bearer " + localStorage.getItem("token"), /* /* with the getItem I access the token stored in the local storage  */ 
        }
      });
